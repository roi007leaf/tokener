...
export function getSourceFilterLabel(options, selectedSourceIds) {
  const selected = new Set(selectedSourceIds ?? []);
  if (!selected.size || selected.size >= (options?.length ?? 0))
    return localize('HUD.AllSources', 'All sources');
  if (selected.size === 1) {
    const [id] = selected;
    return (
...
function renderTokenHud(app, html) {
  ...
  root
    .querySelectorAll('.pf2e-tokener-button, .pf2e-tokener-panel')
...
function renderSourceFilter(container, options, selectedSourceIds, onChange) {
  ...
  if (options.length > 2) {
    ...
    controls.append(
      createSourceControlButton(
        localize('HUD.SelectAllSources', 'Select all'),
        selectedSourceIds,
        onChange,
      ),
  ...
  for (const option of options) {
    menu.append(
      createSourceOption(option, options.length, effectiveSelected, selectedSourceIds, onChange),
    );
  }
...
function renderCandidateSections(content, candidates, tokenDocument) {
  ...
  if (pinned.length)
    content.append(
      createSection(localize('HUD.BestMatches', 'Best matches'), pinned, tokenDocument),
    );
  if (broad.length)
    content.append(
      createSection(localize('HUD.SearchResults', 'Search results'), broad, tokenDocument),
    );
}
...
function createCandidateCard(candidate, tokenDocument) {
  ...
  for (const { action, label, tooltip } of getApplyActions()) {
    const button = document.createElement('button');
    button.type = 'button';
    button.textContent = label;
    setTextTooltip(button, tooltip);
